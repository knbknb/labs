...
setwd("/home/knut/Documents/moocs/dataanaly-lifesci")
library(knitr)
...
summary(log2friction ~ type*leg, data=spider)
fit.log <- lm(log2friction ~ type + leg + type:leg, data=spider)
summary(fit.log)
anova(fit.log)

#What is the L2 vs L1 estimate in log2friction for the push samples? R
library(multcomp) ##Available from CRAN
C <- matrix(as.vector(contrast(fit.log, list(type="push",leg="L2"), list(type="push",leg="L1"))$X), 1)
L2vsL1interaction <- glht(fit.log, linfct=C)
summary(L2vsL1interaction)
coefs[7] - coefs[6] ##we know this is also brown -
