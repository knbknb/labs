...
library(knitr)
library(dplyr)
#setwd("~/Documents/moocs/dataanaly-lifesci/edx-genomicsclass-labs/modeling")
...
$$
\frac{0.99 \cdot 0.00025}{0.99 \cdot 0.00025 + 0.01 \cdot (.9975)}  =  0.02 
$$
...
The following simulation is meant to help you visualize Bayes Theorem. We start by randomly selecting 1500 people from a population in which the disease in question has a 0.025% prevalence.

```{r}
set.seed(3)
prev <- 1/4000
## 1500 people from a population in which the disease in question has a 5% prevalence
...
```{r simulation, fig.cap="Simulation demonstrating Bayes theorem. Top plot shows every individual with red denoting cases. Each one takes a test and with 90% gives correct answer. Those called positive (either correctly or incorrectly) are put in the bottom left pane. Those called negative in the bottom right.", fig.width=10.5, fig.height=27.5,echo=FALSE}
cols <- c("grey","red")
people <- expand.grid(1:M,N:1)
allcols <- cols[d+1] ##Cases will be red, color 2
...
plot(people,col=allcols,pch=16,xaxt="n",yaxt="n",xlab="",ylab="",
    main=paste0(nrow(people) * ncol(people), " Population: ",round(mean(d)*100,4),"% are red"))

plot(people,col=positivecols,pch=16,xaxt="n",yaxt="n",xlab="",ylab="",
     main=paste0(paste( length(negativecols[test==1])," people tested positive:",round(mean(d[test==1])*100,2),"% are red")))

plot(people,col=negativecols,pch=16,xaxt="n",yaxt="n",xlab="",ylab="",
     main=paste0(paste( length(positivecols[test==0])," people tested Negative:",round(mean(d[test==0])*100,4),"% are red")))
```
...
[José Iglesias](http://upload.wikimedia.org/wikipedia/commons/thumb/9/98/Jos%C3%A9_Iglesias_on_September_28%2C_2012.jpg/902px-Jos%C3%A9_Iglesias_on_September_28%2C_2012.jpg) is a professional baseball player. In April 2013, when he was starting his career, he was performing rather well:

| Month | At Bats | H | AVG |
|-------|---------|---|-----|
| April | 20      | 9 | .450   |

(20 bats  at beginning of season -can this be extrapolated? No!  relatively high Standard Error, see net eqn.)
...
infile <- "http://seanlahman.com/files/database/lahman-csv_2014-02-14.zip"
#library()
outfile <- basename(infile)
# tmpfile <- tempfile()
 tmpdir <- tempdir()
if(! file.exists(outfile)) {
        download.file(infile,destfile = outfile)
}

##this shows us the files contained in the zip file
filenames <- unzip(outfile,list=TRUE)
players <- read.csv(unzip(outfile,files="Batting.csv",exdir=tmpdir),as.is=TRUE)
unlink(tmpdir)
#file.remove(tmpfile)
```
